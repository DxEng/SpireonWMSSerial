...
define( [ 'N/ui/serverWidget','N/url','N/runtime','SuiteScripts/DevLibrary/Spireon-Utils','N/record', 'N/query','SuiteScripts/SpireonWMSSerial/spr_util_shipconfirmvalidations' ], main );


function main( serverWidget, url,runtime,Utils,record, query, iffvalidateutil ) {
	...
	function createFrom(context){
		...
		let isItemQtyDeviceQtyMatch = true;
		let deviceQty = 0;
		let isPost = false;
		...
		var form = serverWidget.createForm({ title: 'Validate Single Fulfillment(Dev)' });
		...
		*/
		...
		if (hasDevices){		
			iffvalidateutil.ValidateAndUpdate(iffid,true);		
			...
			var dslCount= iffRec.getLineCount({ sublistId: "recmachcustrecord_spr_srno_transno" }); //device serial line count
			for(var loop_i = 0; loop_i < dslCount; loop_i++) {
				deviceSubList.setSublistValue({
					id: 'serial', line: loop_i, value: iffRec.getSublistValue({sublistId: "recmachcustrecord_spr_srno_transno", fieldId: "custrecord_spr_srno_devsrno", line:loop_i})
				});
				deviceSubList.setSublistValue({
					id: 'error', line: loop_i, value:  iffRec.getSublistValue({sublistId: "recmachcustrecord_spr_srno_transno", fieldId: "custrecord_spr_srno_nspire_err_msg", line:loop_i})
				});				
			}
			*/
		}
		if (isValidIFFPacked && !hasDevices && isUpdateAlso){
			iffvalidateutil.saveIFF(iffid,'C','NOT APPLICABLE');			
			iifstatusfield.defaultValue =  'Shipped';
			nspirestatusfield.defaultValue =  'NOT APPLICABLE';
		}
		return form;
	}
